 ...
 * @LastEditTime: 2023-04-18 11:15:46
-->
<script>
  ...
  import { ornamentUserCurrent } from "@/service/achievement/index";
  ...
  export default defineComponent({
    ...
    setup(_, context) {
      ...
      const state = reactive({
        ...
        userInfo: {}, // 用户排行榜信息
        rankLoading: true,
        avatarUserCurrent: "", // 使用的头像框
        badgeUserCurrent: "" // 使用的徽章
      });

      const queryUserCurrentOrnament = () => {
        ornamentUserCurrent()
          .then(({ code, data, message }) => {
            if (code === 0) {
              data.forEach(item => {
                if (item.ornamentType === "T") {
                  state.avatarUserCurrent = item.ornamentImgUrl;
                } else if (item.ornamentType === "H") {
                  state.badgeUserCurrent = item.ornamentImgUrl;
                }
              });
            } else {
  ...
  <div class="containter">
    ...
    <div class="page-box">
      <div class="bg-box">
        <!-- 用户信息 -->
        <div class="user-box">
          <div class="avator-box" @click="jumpTo('/ornament')">
            <img class="userHead" :src="userInfo.userHead" />
            <img v-if="avatarUserCurrent" class="avatarImg" :src="avatarUserCurrent" />
          </div>
          ...
          <div class="user-flex-box">
            <div class="username">
              {{ userInfo.userName }}
              <!-- 聚合永间 -->
              <img v-if="badgeUserCurrent" class="badge" :src="badgeUserCurrent" @click="jumpTo('/ornament?type=H')" />
              <img
                class="qRCode"
