  ...
  <div class="pub-container">
    <div class="content">
      <van-field
        ...
        autosize
        type="textarea"
        placeholder="输入#内容#可自动生成话题，最多输入十个汉字"
      >
        <div slot="label" @click="handleViewActive" v-if="activityTitle" class="active-title">
          @{{ activityTitle }}
        </div>
      </van-field>
      <div>
      ...
      <div class="operate">
        <div class="select_btn">
          <div @click="handleSelectActivity" class="topic_btn">
            <img
...
<script>
  ...
  export default {
    ...
    watch: {
      text(value, oldValue) {
        if (this.activityTitle) {
          if (value.indexOf(`@${this.activityTitle} `) == -1) {
            // 清理表层
            this.activityTitle = "";
            // 重置下层文字
            this.text = this.bakStr;
          } else {
            this.bakStr = value.split(`@${this.activityTitle} `)[1];
          }
        } else {
          this.bakStr = value;
        }
    ...
    methods: {
      handleViewActive() {
        this.zgStatistics("健康新奥-健康活动-活动动态-发表时点击活动按钮", {
          活动名称: this.activityTitle
        });
        this.$router.push({
          name: "dynamicDetail",
      ...
      handlePublish() {
        ...
        // 过滤话题之前备份输入的内容
        let bakStrTemp = this.bakStr;

        // 过滤的正则
        var reg = /#([^#]+)#/g;
        let temp = this.bakStr.match(reg);
        //   话题内容
        let topicContent = "";
        // 如果有话题的话进行下面操作
        if (temp) {
          for (let index = 0; index < temp.length; index++) {
            ...
            if (element.length > 12) {
              Toast("话题不能超过十个字，请您修改后再发布！");
              this.bakStr = bakStrTemp;
              return;
            }
            // 展示输入的内容
            this.bakStr = this.bakStr.replace(element, "").trim();
            // 展示的话题
            topicContent += element.replace(/#/g, "") + "#";
          }
        ...
        this.zgStatistics("健康新奥-健康活动-活动动态-点击发表按钮", {
          活动名称: this.activityTitle,
          活动分类: type,
          话题名称: topicContent,
          话题分类: "文字"
      ...
      handleSelectTopic() {
        if (this.$route.query.topicTitle) {
      ...
      pageInit() {
        ...
        if (query.topicTitle) {
          this.text = `${this.text}#${decodeURI(query.topicTitle)}#`;
