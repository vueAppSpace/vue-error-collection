<script>
  ...
  import { fetchSportCenterList, getCounselor, insertRecord } from "@/service/service";
  ...
  export default defineComponent({
    ...
    setup(_, ctx) {
      ...
      const getSportList = async () => {
        try {
          const req = { status: 1, pageIndex: 1, pageSize: 10 };
          const { code, data, message } = await fetchSportCenterList(req);
          if (code === 0) {
...
<template>
  <div class="page-box">
    <Banner class="banner" v-if="isEntropy" />
